...
def r_surface_from_data(n, t, u, coords, interp=True, kind="linear", grid_like=False):
    ...
    if grid_like:
        ...
        points_S_u = np.random.rand(n) * cum_S_u[-1]
        N_points = n

    # Find corresponding t-values by interpolation
    rand_t = interp1d(cum_S_t, t[0, :])(points_S_t)
    rand_u = interp1d(cum_S_u, u[:, 0])(points_S_u)

    if interp:
        # Interpolate coordinates, e.g. if func unknown

        rand_coords = np.empty([coords.shape[0], N_points])
        ...
        return rand_coords, rand_t, rand_u, points_S_t, points_S_u

    else:
        return rand_t, rand_u, points_S_t, points_S_u


def r_surface(n, func, t0, t1, u0, u1, t_precision=25, u_precision=25, grid_like=False):
    """
    Parameters
    ----------
    n : int
