...
class MultiPlaneAquarium(SinglePlaneAquarium):
    ...
    __doc__ += SinglePlaneAquarium.__doc__
    ...
    def compute_image_point_currents(self, fish):
        ...
        assert currents_mag.shape[0] == currents_loc.shape[0], 'Magnitudes and locations of point currents numbers should match.'
        ...
        for deg in (np.arange(self.degree)+1):
            ...
            for k in images_dict[str(deg-1)]:
                last_reflection_id = int(k.split('<>')[-1])  # keep track of the last reflection, because we cannot reflect current image with respect to this boundary
                for bi, bound in enumerate(self.boundaries):
                    if bi != last_reflection_id:
                        ...
                        images_dict[str(deg)][k_current] = images_dict[str(deg-1)][k] - 2 * ((images_dict[str(deg-1)][k] - bound.get_point()) @ bound.get_normal()).reshape(-1,1) * bound.get_normal()
    ...
    def details(self):
        ...
        details_string += 'This aquarium type has multiple planar boundaries. It expands on the SinglePlaneAquarium class.'
